...
def train_vit(
    ...
    maximizer_dataset_str = "test" if not use_val_for_model_selection else "val"
    ...
    for epoch in range(epochs):
        ...
        if use_val_for_model_selection:
            if val_results["accuracy"] > best_acc:
                ...
                print(f'\tSaving best model to {savepath}, with {maximizer_dataset_str} accuracy {val_results["accuracy"]:.4f} (at epoch {epoch+1})')
                torch.save(model, savepath)
        else:
            if test_results["accuracy"] > best_acc:
                best_test_results = test_results
                best_acc = test_results["accuracy"]
                print(f'\tSaving best model to {savepath}, with {maximizer_dataset_str} accuracy {test_results["accuracy"]:.4f} (at epoch {epoch+1})')
                torch.save(model, savepath)
